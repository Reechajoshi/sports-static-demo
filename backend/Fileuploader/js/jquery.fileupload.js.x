...
//-----------------------------------
var allowedExtension = [ <?php
		if( isset( $_GET[ 'uplt' ] ) )
		{
			include ( "../../conf/vars.php" );
			echo( $ALLOWED_IMAGE_EXT );	
		}
	?> ];
//-----------------------------------
(function ($) {
    ...
    $.widget('blueimp.fileupload', {
        ...
        options: {
            ...
            formData: function (form,fl_name) {
        ...
        _getFormData: function (options) {
            ...
            if (typeof options.formData === 'function') {
				return options.formData(options.form,options.files[0].name);
        ...
        _onAdd: function (e, data) {
			//-----------------------------------
			var flname = data.files[ (data.files.length - 1) ].name,
				splitName = flname.split( "." ),
			...
			if( extension ) 
			{
				for( cnt=0 ; cnt<allowedExtension.length ; cnt++ )
				{
					if( extension == allowedExtension[cnt] )
					{
						canContinue = true;
						break;
					}
			...
			if( !canContinue )
			{
				alert( "Sorry, such files are not allowed" );
				return;
			}	
